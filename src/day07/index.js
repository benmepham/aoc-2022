...
const parseInput = (rawInput) => {
  ...
  for (let line of input) {
    ...
    } else if (lineArr[0] === "$" && lineArr[1][0] === "l") {
      read = true;
      pathStr = path.join(".");
      tree[pathStr] = 0;
    }
...
const part1 = (rawInput) => {
  const input = calcDirSizes(parseInput(rawInput));
...
const part2 = (rawInput) => {
  ...
  const spaceNeeded = 30000000 - (70000000 - input["/"]);
  const sortable = Object.entries(input).sort(([, a], [, b]) => a - b);

  for (let item of sortable) if (item[1] > spaceNeeded) return item[1];
  return null;
};
